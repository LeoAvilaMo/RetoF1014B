...
for i = 1:length(theta) %necesitamos un vector de posición para el aro
    ...
    campoxtot = [campoxtot campo_Bx];
    campoytot = [campoytot campo_By];
    campoztot = [campoztot campo_Bz];
    
end
CampoTotX = [];
CampoTotY = [];
CampoTotZ = [];
for i = 1:length(campoxtot)
    CampoTotX = CampoTotX + campoxtot(i);
end
disp(CampoTotX)
for i = 1:length(campoytot)
    CampoTotY = CampoTotY + campoytot(i);
end
for i = 1:length(campoztot)
    CampoTotZ = CampoTotZ + campoztot(i);
end
figure;
%Graficamos el aro
x = radio * cos(theta);
...
hold on 
quiver3(X,Y,Z,CampoTotX,CampoTotY,CampoTotZ)